...
Nonlinear Programming (NLP) Optimal Power Flow solver with discrete variables from tap changers and shunt elements
...
from pyomo.core.base import Var, Objective, ConcreteModel, Constraint, minimize, Set, cos, sin, value, Binary, Integers
...
def solveropfnlp_5(ppc, solver="bonmin"):
    if solver == "ipopt":
    ...
    model.tap = Var(model.taps, within=Integers, bounds=(tapmin, tapmax))

    # Shunt susceptance
    model.Bs = Var(model.shunt)

    # Shunt positions + their bounds
    model.s = Var(model.shunt, within=Integers, bounds=(0, stepmax))
